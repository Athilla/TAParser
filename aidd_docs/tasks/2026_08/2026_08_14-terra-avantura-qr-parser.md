...
### #9 - 2026-08-14T15:40:00Z
> Bug report (post-delivery): user found that neither the parcours list nor comments were being scanned in the real run.
> Root cause (debug skill, 4 hypotheses validated): `ParcoursService` scraped `<article class="gc-caches-liste">` elements that **never exist in the raw HTTP response** of `/parcours` (anonymous or authenticated) - that markup is generated client-side by JavaScript from a JSON blob, so `GetAllParcoursAsync` always returned an empty list, so the main loop never had anything to iterate, so `CommentService` was never called either. Confirmed via `curl` (0 matches for `gc-caches-liste`/`js-gc-map-nid-`) and via a real page download parsed offline.
> Also discovered along the way: the real parcours data (nid, title, field_city, department...) IS present in the raw HTML, embedded in Drupal's standard `<script type="application/json" data-drupal-selector="drupal-settings-json">` block, under `geocaching_map.markers` (mixed `geocaching_cache` and `partners_places` entries - must filter by `type`). Also confirmed cache pages (`/caches/...`, `/node/{nid}`) redirect anonymous requests to the CGU page (`/node/386`) but serve content directly once authenticated - already satisfied since `ParcoursRunner` logs in before calling `ParcoursService`/`CommentService` on the same shared `HttpClient`/`CookieContainer`.
> 🤖 Fix: rewrote `ParcoursService.ExtractParcours` to parse `geocaching_map.markers` from the embedded JSON via `System.Text.Json` (new private `GeocachingMarker`/`GeocachingMapSettings`/`DrupalSettingsRoot` records) instead of XPath-scraping HTML articles. `ParcoursReference.Url` is now built as `/node/{nid}` (Drupal redirects to the canonical `/caches/{slug}` alias, verified working). Updated the fixture (`parcours-listing.html`, now a realistic `drupalSettings` JSON snippet) and `ParcoursServiceTests.cs` (4 tests: count, field/URL extraction, `partners_places` exclusion, missing-script case).
= ✓ `dotnet build` : 0 errors/warnings. `dotnet test` : 17/17 passed. Additionally sanity-checked (temporary test, removed after) against a real downloaded `/parcours` page: **607 parcours extracted**, matching the ~600 expectation, first entry correctly parsed (nid=25, "Graniticache", Meymac). `CommentService`'s existing XPath selectors were already verified to work against real authenticated cache-page HTML (11 `js-comment` matches via `curl` with a real session cookie), so no change was needed there - the bug was isolated to `ParcoursService`.
→ Fix ready to commit/push. User to re-validate end-to-end on their machine (network still blocked in this sandbox).
